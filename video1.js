...
(function () {
  ...
  /* ─────────────────── POINTER SWIPE SEEK ─────────────────── */
  const swipe = { active: false, video: null, startX: 0, startY: 0, startTime: 0, lastUpdate: 0, cancelled: false, pointerId: null };
  ...
  const onPointerDown = e => {
    // Only handle touch input (not mouse/pen)
    ...
    const video = getVideoAtPoint(e.clientX, e.clientY);
    if (!video?.duration) return;

    // Safe zone: bottom 15%
    const rect = video.getBoundingClientRect();
    if (e.clientY > rect.bottom - rect.height * 0.15) return;
  ...
  const onPointerCancel = e => {
    if (e.pointerId === swipe.pointerId) {
      swipe.active = swipe.cancelled = false;
      swipe.video = null;
      swipe.pointerId = null;
    }
  ...
  // Use Pointer Events for better browser compatibility
  ...
  // Prevent touch-action on videos to ensure pointer events work
  GM_addStyle('video { touch-action: pan-y pinch-zoom; }');
