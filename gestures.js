...
(() => {
  ...
  const STORE_KEY = 'vmug_cfg_v165';
  const DEFAULTS = {
    lpress:   { enabled:true, mode:'bg', longMs:500, tapTol:24 }, // long-press: chỉ mở LINK
    rclick:   { enabled:true, mode:'fg' },                        // mặc định FG
    dblRightMs: 600,
    triTapMs:   900                                               // mặc định 900 ms
  };
  ...
  GM_registerMenuCommand?.(`👆 Triple tap (ms): ${CFG.triTapMs}`, () => {
    ...
    if (Number.isFinite(v) && v>=150 && v<=2000){ CFG.triTapMs=v; saveCfg(); alert('Saved.'); }
  });
  ...
  addEventListener('contextmenu', ev => {
    ...
    const a=getAnchorFromEvent(ev); if(!validLink(a)) return; // không phải link: giữ menu gốc
    ev.preventDefault(); ev.stopPropagation();
    openByMode(a.href, CFG.rclick.mode);
    blockNextContextmenuUntil = now+600;
  }, true);

  /* ===== TRIPLE TAP (touch) → CLOSE TAB (không multi-touch guard/window) ===== */
  let taps=[];
  addEventListener('touchstart', ev => {
    if(inEditable(ev.target)) return;
    if(ev.touches.length>1) return; // bỏ qua đa ngón theo yêu cầu
    const now=Date.now();
